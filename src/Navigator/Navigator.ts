...
export default class Navigator {
  ...
  async checkPageLang(expectedLang: string): Promise<boolean> {
    const isVisible = await this.page.locator(`div.header__locales div[value=${expectedLang}]:nth-of-type(1)`).isVisible()

    if (isVisible == true){
      console.log(`Page language is ${expectedLang}`);
      return true;
    } else {
      console.log(`Page language is not ${expectedLang}`);
      return false;
    }
  }

  async changeLangIfNeeded(callback: boolean, expectedLangIndex: number){

    if (await callback == false){
      await this.page.locator('#lang_dropdown').click()
      await this.page.locator(`#lang_dropdown-item-${expectedLangIndex}`).click()
    }
  }
}
